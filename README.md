...
1. Administrative history data model reconstruction - inputs:
   ...
   d. changes_list.json file (list of administrative changes). Each change is a part of legal act coded into a machine-readible format and quotes its legal source. The data models of administrative changes are defined in the data_models.adm_change file.
...
On the basis of these inputs the toolkit is able to reconstruct full administrative history of of a country (group of countries) on the Country -> Region -> District level and generate maps for every administrative state. It can generate the corcondance matrices to harmonize data between any two chosen administrative states. If the inputs are too scarce (e.g. many administrative changes occured and only 3 maps were loaded) and the toolkit is unable to deduce shapes of some districts, it uses fallback territories of the districts from other timepoints or other fallback methods to generate the harmonization matrices.
