...
Each tool in the toolkit uses different inputs. Their structure is defined by appropriate pydantic data models (PDM) for input data validation purposes:

1. Administrative history data model reconstruction with `core.core.AdministrativeHistory` instance:
    ...
    **Ready Example Inputs**
    A ready example of inputs summarizing the administrative history of Poland between 1921 and 1939 is stored in the `data/adm_histories/interwar_poland` folder.
...
2. Standardization of district-level datasets with `visualization.streamlit_app` GUI:

    **Inputs**
    a. CSV with the columns `"District"` and/or `"Region"` with economic/social data.

    The visualization GUI uses a ready `core.core.AdministrativeHistory` instance.
    
    **Comments**
    The Streamlit app provides a GUI for the easy standardization of data to align the District and/or Region naming conventions and choose the administrative state that the dataset fits. A standardized dataset can be directly used as an input for the harmonization step.

3. Processing and harmonization of datasets with `core.processor.AdministrativeHistoryProcessor` instance:
    ...
    **Inputs**
    ...
    b. `processing_config.json` (in the current version, holding only a list of post-processing methods to apply). The PDMs for the processing config and each post-processing method dict are defined in the `data_models.processing_config` module.

    **Ready Example Inputs**
    A ready example of inputs used for the data imputation and harmonization of district-level data to chosen borders is stored in the `data/datasets/interwar_poland_database` folder together with a collection of district-level datasets.

    **Comments**
    This processing and harmonization tool builds on top of a ready `core.core.AdministrativeHistory` instance and uses standardized datasets as inputs. The visualization tool is recommended for the preparation of the standardized inputs.

    In the current version of the toolkit, the harmonized datasets are stored in CSV format due to the limited size of the database and simpler hosting of the planned database website. The modul can be very simply rewritten to connect with an SQL database.

4. Plotting of administrative history summaries:

    This plotter uses a ready `core.core.AdministrativeHistory` instance.

5. Access to datasets with `core.api.AdministrativeHistoryAPI` instance:

    This API builds uses a ready `core.processor.AdministrativeHistoryProcessor` instance.
   
## Understanding the Data Model of an Administrative History
Generally put, the administrative history of each country can be analyzed from two points of view:
1. administrative units: their creation and abolishment through time;
2. hierarchic relations between units: the administrative hierarchy's changes through time.
This is why in the toolkit the basic element of analysis is the `UnitState` PDM (and its children `DistrictState` and `RegionState` PDMs). `Unit` PDM (`District` or `Region` PDM) represents an administrative unit and holds a list of UnitState instances (`District` instance holds a list of `DistrictState` instances, `Region` instance holds a list of `RegionState` instances). `AdministrativeState` PDM holds a "country_name->region_name->district_name" hierarchy of `Unit` names.
...
2. `OneToMany`: transfers some territory from one `District` to at least one `District`. It creates a new `AdministrativeState` (even if the hierarchy between regions and districts doesn't change). It can abolish the district from which the territory is taken (i.e. end the timespan of the `DistrictState` of the district) or create a new district to which the territory is transfered (i.e. add a new `District` instance to the `DistrictRegistry` instance with a new `DistrictState` instance, and add the district to the `AdministrativeState`). `OneToMany` change attributes do not "define" the exact way in which the territory is transfered - the only effect on the knowledge about the territories is that during the stage of reconstruction of maps for each administrative state the `AdministrativeHistory` object "knows" that a transfer between the territories occured and that the exact form of the transfered has to be deduced from the maps.
